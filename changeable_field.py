...
import random                   #Seeds (randomness)

#-------------------------------------------------
def changeable_field (N_ex, time_steps, step, t, B, x, y, save, replicable_exp, starting_mode, option):
    ...
    #Controlling the size of matrix
    if time_steps <= 40:
        size = 1
    else:
        size = 0.03
    
    changes = int (size*time_steps)         #Sets the size of some matrixs
        ...
        for j in range(1,time_steps):
            ...
            if j == 1:      #Only for the second step
                ...
                if random_n [j]<0.01:  #Given the condition:
                    ...
                    Rt [i,b] = R_time [i,j-1]                   #Saves the previous Res Time
    ...
    if save == 1:
        ...
        R_time_copy.to_csv('R_time' + extension, index=False)

        Rt_copy = pd.DataFrame(Rt)
        Rt_copy.to_csv('Rt' + extension, index=False)
        
        Rt_1_copy = pd.DataFrame(Rt_1)
        Rt_1_copy.to_csv('Rt1' + extension, index=False)
        
        Rt_0_copy = pd.DataFrame(Rt_0)
        Rt_0_copy.to_csv('Rt0' + extension, index=False)
        
        state_0_copy = pd.DataFrame(state_0)
        state_0_copy.to_csv('state_0' + extension, index=False)
        
        state_1_copy = pd.DataFrame(state_1)
        state_1_copy.to_csv('state_1' + extension, index=False)

        Rt_final_0_copy = pd.DataFrame(Rt_final_0)
        Rt_final_0_copy.to_csv('Rt_final_0' + extension, index=False)
        
        Rt_final_1_copy = pd.DataFrame(Rt_final_1)
        Rt_final_1_copy.to_csv('Rt_final_1' + extension, index=False)
        
        changes_m_copy = pd.DataFrame(changes_m)
        changes_m_copy.to_csv('changes' + extension, index=False)
