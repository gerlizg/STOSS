...
def curve_fitting (name_plot, T, ratio1, ratio2, name, number, x, y, P_i, graph_type, option, N_ex, step, B, time_steps, E, P_ij):
    ...
    if graph_type == 1:        #For residence times
        ...
        if option == 1:
            ...
            ax1.set_ylabel('Relaxation curve', color=color)
            ax1.plot(x, y, color=color)
            ax1.tick_params(axis='y', labelcolor=color)
            ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis
            
            color = 'tab:blue'
            ax2.set_ylabel('Magnetic Field', color=color)  # we already handled the x-label with ax1
            ax2.plot(x, -1*B, color=color)
            ax2.tick_params(axis='y', labelcolor=color)
            
            
            fig.tight_layout()  # otherwise the right y-label is slightly clipped
            plt.title('Relaxation plot for %d experiments at %d mK. Compound: %s (Sample ID: %s)' % (N_ex, T*1000, name, number), fontsize = 25)
            plt.grid()
            plt.savefig(name1)
            plt.show()
            ...
            plt.rcParams["figure.figsize"] = [18, 14]
            plt.rcParams["figure.autolayout"] = True
            plt.plot(x, y, linewidth = 1.5, color = "red", label= "Real")
            plt.plot(x, y_fitted, linewidth = 1.5, color = "blue", label=r'Fitted function: $y= (%.2f)e^{(-x/%.4f)}+%.2f$' %(a, tau, c))
            plt.xlabel("Time(s)")
            plt.ylabel("Experiments")
            plt.title('Relaxation plot for %d experiments at %d K. Compound: %s (Sample ID: %s)' % (N_ex, T, name, number), fontsize = 25)
            plt.legend(loc='upper right')
            plt.grid()
            plt.savefig(name1)
            plt.show()
